...
def start():
    if request.method == "POST":
        ...
        referral_index = -1
        if code:
            rows = db.execute("SELECT id FROM users WHERE referral_code=?", code)
            if len(rows) == 0:
        ...
        for i in range(len(session["selected_words"])):
            ...
            if (request.form.get(session["selected_words"][i])):
                if(len(session["remaining_word_indexes"]) < 1):
                    ...
                    break
                new_word_index = math.floor(random() * len(session["remaining_word_indexes"]))
...
def experiment():
    ...
    for i in range(math.floor(len(session["selected_words"])/2)):
        ...
        session["first_treatment_words"].append(session["selected_words"].pop(index))
    session["second_treatment_words"] = session["selected_words"]
    first_treatment_pairs = []
    for word in session["first_treatment_words"]:
        rows = db.execute("SELECT definition from words WHERE word=?", str(word))
...
def finish():
    if request.method == "POST":
        ...
        for word in session["second_treatment_words"]:
            second_answers_list.append({"word": word, "ans": request.form.get(word)})
        code = make_code()
        tickets = 1
        ...
        for item in session["first_answers"]:
            if(not item["ans"]):
                continue
            rows = db.execute("SELECT definition FROM words WHERE word=?", item["word"])
...
def experiment2():
    if request.method == "POST":
        first_answers_list = []
        for word in session["first_treatment_words"]:
            first_answers_list.append({"word": word, "ans": request.form.get(word)})
